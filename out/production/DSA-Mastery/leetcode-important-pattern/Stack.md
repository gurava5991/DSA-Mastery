...
> **Note:**
> - `Stack` is a legacy class (inheriting from `Vector`) and is **synchronized** (thread-safe, but slower).
> - For single-threaded applications, **`ArrayDeque`** is recommended as a **faster alternative**.
...
| `search(Object)` | Returns the **position** of an element from top  | O(n)            |
...
### 3. Stack Implementation Using Queues

There are two approaches to implementing a stack using  queue:
